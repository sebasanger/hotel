  ...
  <div>
      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header">
            <button class="btn-success float-right" @click="newModal">Agregar producto</button>
            <div class="input-group input-group-sm" style="width: 200px;">
              <select v-model="search" name="categorias_id" @change="buscar()" class="form-control">
                <option value>Filtrar por categoria</option>
                <option v-for="(c,index) in categorias" :key="index" :value="c.id">{{ c.categoria}}</option>
              </select>
            </div>
          </div>
            ...
            <table class="table table-hover">
              ...
              <tbody>
                <tr v-for="item in productos.data" :key="item.id">
                  ...
                  <td>
                    <button @click="ingresoModal(item.id)" class="btn">
                  ...
                  <td>
                    ...
                    |
                    <button class="btn" @click="deleteProducto(item.id)">
          ...
          <div class="card-footer" v-show="!search">
            <pagination :data="productos" :limit="3" @pagination-change-page="getResults"></pagination>
          </div>
        </div>
        <!-- /.card -->
      </div>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5
              class="modal-title"
              id="addNew"
              v-text="editMode ? 'Editar producto' : 'Crear producto'"
            ></h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <form @submit.prevent="editMode ? updateProducto() : createProducto()">
            <div class="modal-body">
              ...
              <div class="form-group">
                ...
                <input
                  ...
                  placeholder="Codigo"
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('codigoProducto') }"
                />
                <has-error :form="form" field="codigoProducto"></has-error>
              </div>
              ...
              <div class="form-group">
                ...
                <select
                  ...
                  name="marcas_id"
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('marcas_id') }"
                >
                  <option value>Seleccionar una marca</option>
                  <option v-for="(m,index) in marcas" :key="index" :value="m.id">{{ m.marca}}</option>
                </select>
                <has-error :form="form" field="marcas_id"></has-error>
              </div>
              ...
              <div class="form-group">
                ...
                <select
                  ...
                  name="categorias_id"
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('categorias_id') }"
                >
                ...
                <has-error :form="form" field="categorias_id"></has-error>
              </div>
            </div>
    ...
    >
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="addNewIngreso" v-text="'Nuevo ingreso de producto id: '+ formIngreso.productos_id"></h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
            ...
            <div class="modal-body">
              ...
              <div class="form-group">
                ...
                <input
                  ...
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('precioCompra') }"
                />
                <has-error :form="form" field="precioCompra"></has-error>
              </div>
              ...
              <div class="form-group">
                ...
                <input
                  ...
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('cantidadIngreso') }"
                />
                <has-error :form="form" field="cantidadIngreso"></has-error>
              </div>
              ...
              <div class="form-group">
                ...
                <select
                  ...
                  name="modosPagos_id"
                  class="form-control"
                  :class="{ 'is-invalid': form.errors.has('modosPagos_id') }"
                >
                  <option value>Seleccionar un modo de pago</option>
                  <option v-for="(m,index) in modosPagos" :key="index" :value="m.id">{{ m.modoPago}}</option>
                </select>
                <has-error :form="form" field="modosPagos_id"></has-error>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-danger" data-dismiss="modal">Cancelar</button>
...
export default {
  ...
  methods: {
    ...
    loadCategorias() {
      axios.get("categoria").then(res => (this.categorias = res.data.data));
    },
    loadMarcas() {
    ...
    getResults(page = 1) {
      axios.get("producto?page=" + page).then(res => {
    ...
    filter() {
      ...
      axios.get("findProducto/" + query).then(res => {
        this.productos = res.data;
      });
    },
